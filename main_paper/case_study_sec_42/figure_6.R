...
## For the main paper, we're showing the _second_ moment analysis,
## roughly analogous to the covariance estimated by classical PCA.
## This doesn't differ much from the first moment structure for
## this particular data set, but it's maybe a bit more familiar to readers

# To get the second moment, we will re-perform SST-PCA on the residuals
# after removing the first moment
#
# To make this most similar to a graph, we'll re-binarize the edges
...
# Here, the scree plot suggests a (slightly) higher rank than for M1
if(interactive()){
...
## Figure 6: Big plot of all results
# -1) Scree Plots for R, K
# -2) CD on SST_PN
# -3) CD on Aggregated
# All relevant terms here are "M2_" prefixed since we're visualizing the
# second moment
...
M2_G1 <- TBL |>
    select(Hour, Day, HD) |>
    distinct() |>
    mutate(U = RESID_FITS[[RESID_R_HAT]]$u_hat) |>
    ggplot(aes(x=HD, y=U)) +
    geom_line(group=1) +
...
RESID_GRAPH      <- graph_from_adjacency_matrix(abs(RESID_FITS[[RESID_R_HAT]]$V_hat_w) > 2e-1, mode="undirected")
...
as_tbl_graph(RESID_GRAPH) |> activate(nodes) |>
    ...
    guides(color = guide_legend(override.aes=list(size=2))) -> M2_G2

RESID_ANY_MAT <- apply(RESID_TENSOR, 1:2, any)
RESID_ANY_GRAPH <- igraph::graph_from_adjacency_matrix(RESID_ANY_MAT, mode="undirected")

as_tbl_graph(RESID_ANY_GRAPH) |> activate(nodes) |>
    mutate(ix = row_number()) |>
    left_join(ID_IX_MAP, join_by(ix==IX)) |>
    mutate(CD=cluster_leading_eigen(RESID_ANY_GRAPH)$membership) |>
    left_join(META, join_by(ID == ID)) |>
    ggraph(layout="kk") +
    geom_node_point(aes(color=Class), size=2.5) +
    scale_color_discrete(name="Student Class (Outer)") +
    ...
    guides(color = guide_legend(override.aes=list(size=2)))-> M2_G3

data.frame(rerr=sapply(RESID_FITS, function(f) sum(f$X_hat^2) / sum(TENSOR^2)),
           rank=seq(R_MAX)) |>
    ggplot(aes(x=rank, y=rerr)) + geom_point(size=2) + geom_line() + xlab("Rank of V Factor") +
    ylab("%VE") +
    theme_bw() +
    geom_vline(xintercept=RESID_R_HAT, linetype=2, linewidth=2, color="red4") +
    #ggtitle("Scree Plot for Selection of SST-PCA Rank") +
    scale_y_continuous(labels=scales::percent)-> M2_G4

M2_G_COMP <- (M2_G2 + M2_G3 + plot_layout(guides="collect") & theme(legend.position="bottom") &
                  theme(legend.text = element_text(size = 14),
                        legend.title=element_text(size=16)))+
    (M2_G1 / M2_G4 + plot_layout(guides="keep") & theme(axis.text = element_text(size=12),
    )) +
    plot_annotation(tag_levels="A")

ggsave("figure_6.pdf", M2_G_COMP, width=18, height=5)
if(grepl("darwin", version$os, ignore.case=TRUE)){
    system("open figure_6.pdf")
} else {
    system("xdg-open figure_6.pdf")
}
